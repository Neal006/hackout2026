...
## Member A — Scheduler and proof (the number)
...
**h30–48**
- Support B on the four demo scenarios (they all exercise the solver). Tune weights so the demo numbers are honest and visible.

Hands to B: `solve()`, `impact()`, `price()` signatures frozen by h10.

---

## Member B — Simulator, API, OCPP (the engine room)
...
- Stub `solve()` returning full power so the loop runs before A's solver lands.
...
- Fail-safe ladder: live signal → cached (≤6 h) → tariff-only → deadline-only → full power. Exposed as `GET /sites/{id}/status.mode` so C can show a banner.
...
- `POST /demo/early_unplug` (Tom leaves at 13:00), `POST /demo/boost` (Sofia), `POST /demo/oversubscribe` (+20 late arrivals), `POST /demo/signal_outage` (kill the feed → ladder drops). Each returns what changed so C can show it.

**h40–48**
- Seed script, `README.md` run instructions, freeze the demo dataset.

Hands to C: OpenAPI schema and WebSocket message shapes frozen by h12.

---

## Member C — Front-ends (what the judges see)

Owns `web/` — React + Vite, one code base, two routes: `/driver` (PWA) and `/ops`.

**h0–6**
- Scaffold, routing, WebSocket client, mock data matching B's message shapes. Colour system: warm off-white, amber accent, grid-blue for data.
...
- Small line under the question: "your last 5 visits: left 17:20–17:45" (from B's session history).
...
- A "Demo" drawer on `/ops` with four buttons wired to B's `/demo/*` endpoints. Each button must produce a change the audience can see within two seconds (Gantt reshuffle, banner, ETA update).
...
## Member D — Data, pitch, demo (the story)

Owns `pitch/`, `README.md` narrative, demo choreography, and unblocking everyone else.

**Before the clock**
- Back up A on registrations: WattTime, ACN-Data, gridstatus install. Pick the demo day (a spring weekday with strong midday curtailment) and confirm both datasets cover it.
...
**h16–30**
- Demo script, timed to 4 minutes: (1) plug-in question, (2) dashboard fills as the day plays, (3) Boost, (4) early unplug, (5) oversubscribe, (6) signal outage, (7) receipt. Rehearse against C's mock data before the real thing exists.
...
| Hour | Gate | Owner |
|---|---|---|
| 6 | `prove.py` prints ≥15% saving; slide 1 exists | A, D |
| 10 | `solve()` / `impact()` / `price()` signatures frozen | A → B |
| 12 | OpenAPI + WebSocket shapes frozen | B → C |
| 16 | Day plays end to end with stub UI | B, C |
| 30 | Both front-ends live on real data | C |
| 40 | Four demo buttons work; clean-machine run passes | B, C, D |
| 46 | Backup video recorded | D |
