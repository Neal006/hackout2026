  ...
  const data = useMemo(() => {
    ...
    return {
      ...
      portfolio: {
        ...
        limitKw: feedKw,
        peakAvoidedKw: Math.max(0, Math.round(feedKw - Math.max(...ticks.map((t) => t.ev + t.building), 0))),
      ...
      siteDetail: {
        ...
        isOptimizing,
        impact: { saved_usd: impact?.saved_usd ?? 0, saved_kgco2: impact?.saved_kgco2 ?? 0, kwh: impact?.kwh ?? 0, sessions: impact?.sessions ?? 0 },
        meter_ticks: ticks,
        connectors,
      },
      sites: [siteRow],
